...
export class ModificarMotivoComponent {
  ...
  ngOnInit() {
    this.motivosService
      .getMotivo()
      .subscribe({ next: (response) => (this.nombreMotivo = response) });
    this.motivosForm = new FormGroup({
      motivo: new FormControl(this.nombreMotivo, [
        Validators.required,
        this.validarLetras,
      ]),
    });
  ...
  enviarDatos() {
    ...
    this.motivosService
      .modificarMotivo(this.id, this.motivosForm.get('motivo').value)
      .subscribe({
        next: (response) => {
        ...
        error: (error) => {
          if (
            error.error.mensaje === 'Ya existe un motivo con el mismo nombre.'
          ) {
            Swal.fire({
              icon: 'error',
              title: 'Oops...',
