...
export class ModificarPaisComponent {
  ...
  ngOnInit() {
    this.paisService
      .getNombre()
      .subscribe({ next: (response) => (this.nombrePais = response) });
    this.paisService
      .getUrlPais()
      .subscribe({ next: (response) => (this.urlPais = response) });

    this.paisForm = new FormGroup({
      pais: new FormControl(this.nombrePais, [
        Validators.required,
        this.validarLetras,
      ]),
      urlPais: new FormControl(this.urlPais, [
        Validators.required,
        this.validarLetras,
      ]),
    });
