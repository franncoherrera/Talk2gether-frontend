...
import {
  ...
  getUnixTimestamp,
  logger,
} from 'src/cometchat-pro-angular-ui-kit/CometChatWorkspace/src/utils/common';
...
@Component({
  ...
  templateUrl: './comet-chat-conversation-list-screen-page.component.html',
  styleUrls: ['./comet-chat-conversation-list-screen-page.component.scss'],
  animations: [
...
export class CometChatConversationListScreenPageComponent {
  ...
  idChatOtro: string;
  idChatLogueado:string;
  armarUsuario: any;
  blockedForHim: boolean = false;
  blockedForMe: boolean = false;
  ...
  ngOnInit() {
    this.navbarService.getIdLogueado().subscribe({next: (r)=>this.idChatLogueado=r})
    this.router.params.subscribe({
      next: (params) => {
        this.idChatOtro = params['id']; 
      },
    });

    combineLatest([
      this.chatService.consultarUsuario(this.idChatOtro),
      this.chatService.consultarBloqueados(this.idChatOtro),
      this.chatService.consultarBloqueados(this.idChatLogueado)
    ]).pipe(
      switchMap(([user, bloqueadosOtro, bloqueadosPropios]) => {

        const bloqueadosDataOtro = bloqueadosOtro['data'];
        for (let i = 0; i < bloqueadosDataOtro.length; i++) {
          const b = bloqueadosDataOtro[i];
          if(b.uid == this.idChatLogueado){
            this.blockedForHim = true
          }
        }
        const bloqueadosDataPropio = bloqueadosPropios['data'];
        for (let i = 0; i < bloqueadosDataPropio.length; i++) {
          const b = bloqueadosDataPropio[i];
          if(b.uid == this.idChatOtro){
            this.blockedForMe = true
          }
        }
        this.armarUsuario = {
          avatar: user['data']['avatar'],
          blockedByMe: this.blockedForMe,
          conversation_Id: this.idChatOtro+'_user_'+this.idChatLogueado,
          deactivatedAt: this.blockedForHim,
          hasBlockedMe: false,
          lastActiveAt: user['data']['lastActiveAt'],
          name: user['data']['name'],
          role: user['data']['default'],
          status: user['data']['status'],
          uid: user['data']['uid'],
        };
        Swal.fire({
          title: '¿Estás seguro que quieres hablar con '+this.armarUsuario['name']+'?',
          icon: 'warning',
          showCancelButton: true,
          confirmButtonColor: '#3085d6',
          cancelButtonColor: '#d33',
          cancelButtonText: 'Volver al perfil de "'+this.armarUsuario['name']+'"',
          confirmButtonText: 'Si, deseo chatear',
        }).then((result) => {
          if (result.isConfirmed) {
            this.userClicked(true, null);
          } else {
            this.route.navigate(['perfilUsuario', this.idChatOtro])
          }
        });
        return of(null);
      }),
      catchError((error) => {
        Swal.fire({
          icon: 'error',
          title: 'Oops...',
          text: 'El usuario solicitado no se encuentra disponible',
        });
        this.route.navigate(['perfilUsuario', this.idChatOtro])
        return of(null);
      })
    ...
    try {
       ...
       */
      this.CometChatService.conversationDeleted.subscribe(
        (conversation: any) => {
          if (
            this.item &&
   ...
   */
  userClicked(algo: boolean, user1?: any) {
    if (user1 && algo == null) {
      try {
  ...
  groupUpdated = (
    ...
    key: any,
    group: any,
    options: { [x: string]: any; user: { uid: any } }
  ) => {
    try {
      switch (key) {
